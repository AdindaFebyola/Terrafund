...
exports.joinProject = async (req, res) => {
  ...
  if (!user_id || !project_id) {
    return res.status(400).json({ message: 'User ID dan Project ID wajib diisi.' });
  }

  try {
    // 2. Validasi Role User (Harus Relawan)
    ...
    // 3. Validasi Proyek (Apakah proyek ada?)
    const [[project]] = await db.query('SELECT id, title, status FROM projects WHERE id = ?', [project_id]);
    if (!project) {
      return res.status(404).json({ message: 'Proyek tidak ditemukan.' });
    }
    
    // (Opsional) Cek status proyek, misal jika 'completed' tidak bisa join lagi
    ...
    // 5. Eksekusi Insert
    // status default: 'active' (artinya sudah terdaftar aktif)
    // verification_status default: 'pending' (menunggu verifikasi jam kerja nanti)
    // hours default: 0
    await db.query(
      `INSERT INTO user_projects (user_id, project_id, status, hours, verification_status, joined_at) 
...
// ... (Kode endpoint lain seperti getDashboard, getProfile, dll tetap sama di bawah ini) ...
