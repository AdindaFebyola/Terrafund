...
app.post('/api/register', async (req, res) => {
  try {
    ...
    const [existing] = await db.query('SELECT id FROM users WHERE email = ? OR wallet_address = ?', [email, wallet_address]);
    ...
    // Simpan user baru
    const [result] = await db.query('INSERT INTO users (name, email, wallet_address) VALUES (?, ?, ?)', [name, email, wallet_address.toLowerCase()]);
...
app.post('/api/donate', async (req, res) => {
  try {
    ...
    await db.query('INSERT INTO donations (order_id, amount, status) VALUES (?, ?, ?)', [orderId, amount, 'pending']);
...
app.post('/api/notification', async (req, res) => {
  try {
    ...
    if (transactionStatus == 'capture') {
      if (fraudStatus == 'accept') statusDonasi = 'padi';
    ...
    if (statusDonasi === 'padi') {
      // 1. Update Database jadi padi
      await db.query('UPDATE donations SET status = ? WHERE order_id = ?', ['padi', orderId]);
      console.log(`✅ DB Updated: ${orderId} -> padi`);
